...
from spectre_utils import degree_stats, clustering_stats, spectral_stats

class Args_evaluate():
    def __init__(self):
        # loop over the settings

        self.model_name_all = ['HypDiff'] 

        # list of dataset to evaluate
        # use a list of 1 element to evaluate a single dataset
        self.dataset_name_all = ['CL100']
        # self.dataset_name_all = ["MUTAG","Grid","BA","SynCommunity","SynER","SynEgo","PROTEINS","IMDB-BINARY","COLLAB"]
...
def evaluation_epoch(dir_input, fname_output, model_name, dataset_name, args, is_clean=True, epoch_start=1000,epoch_end=3001,epoch_step=100):
    with open(fname_output, 'w+') as f:
        ...
        elif 'GraphRNN_RNN' in model_name:
            fname_test = dir_input + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                hidden) + '_test_' + str(0) + '.dat'
        else:
            fname_test = dir_input + dataset_name + '/graph_test.dat'
        try:
            graph_test = utils.load_graph_list(fname_test,is_real=True)
        except:
            print('Not found: ' + fname_test)
        ...
        if 'GraphRNN_RNN' in model_name or 'GAE' in model_name or 'HGCN' in model_name or 'HypDiff' in model_name:
            # read test graph
            for epoch in range(epoch_start,epoch_end,epoch_step):
                sample_time=1
                # for sample_time in range(1,4):
                # get filename
                if model_name == 'GraphRNN_RNN':
                    fname_pred = dir_input + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(hidden) + '_pred_' + str(epoch) + '_' + str(sample_time) + '.dat'
                else:
                    fname_pred = dir_input + dataset_name + '/diff_embeddings.npy'
                # load graphs
                try:
                ...
                # evaluate MMD test
                print('Compute mmd_degree')
                # mmd_degree = eval.stats.degree_stats(graph_test, graph_pred)
                mmd_degree = degree_stats(graph_test, graph_pred, compute_emd=True)
                print('Compute mmd_clustering')
                mmd_clustering = clustering_stats(graph_test, graph_pred, compute_emd=True)
                # mmd_clustering = eval.stats.clustering_stats(graph_test, graph_pred)
                print('Compute mmd_spectral')
                mmd_spectral = spectral_stats(graph_test, graph_pred, compute_emd=True)
                print('test: degree',mmd_degree,'clustering',mmd_clustering,'spectral',mmd_spectral)
                mmd_4orbits = -1

                # # evaluate MMD validate
                # mmd_degree_validate = degree_stats(graph_validate, graph_pred, compute_emd=True)
                # mmd_clustering_validate = clustering_stats(graph_validate, graph_pred, compute_emd=True)
                # mmd_spectral_validate = spectral_stats(graph_validate, graph_pred, compute_emd=True)
                ...
                # print('validate: degree',mmd_degree_validate,'clustering',mmd_clustering_validate,'spectral',mmd_spectral_validate)
...
if __name__ == '__main__':
    ...
    ## if you don't try kronecker, only the following part is needed
        ...
        evaluation(args_evaluate,dir_input=dir_prefix+"logs/graphtask/", dir_output=dir_prefix+"eval_results/",
