...
def load_graph_list(fname,is_real=True):
    ...
    for i in range(len(graph_list)):
        ...
        if len(list(edges_with_selfloops))>0:
            graph_list[i].remove_edges_from(list(edges_with_selfloops))
        if is_real:
            # graph_list[i] = list(graph_list[i].subgraph(c) for c in nx.connected_components(graph_list[i]))[0]
            graph_list[i] = list(graph_list[i].subgraph(c) for c in nx.connected_components(nx.Graph(graph_list[i])))[0]
            # graph_list[i] = max(nx.connected_component_subgraphs(graph_list[i]), key=len)
            graph_list[i] = nx.convert_node_labels_to_integers(graph_list[i])
        else:
            graph_list[i] = pick_connected_component_new(nx.Graph(graph_list[i]))
    return graph_list
