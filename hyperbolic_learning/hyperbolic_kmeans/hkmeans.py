...
import sys
from .util_hk import norm,hyperboloid_dot,hyperboloid_dist, poincare_pts_to_hyperboloid,poincare_pt_to_hyperboloid,hyperboloid_pt_to_poincare,poincare_dist
...
class HyperbolicKMeans():
    ...
    def init_centroids(self, dim=None, radius=0.3):
        ...
        # print(centers.shape)
        center=np.random.uniform(0, radius, size=(self.n_clusters, dim)) 
    ...
    def fit(self, X, y=None, max_epochs=40, verbose=False):
        ...
        #    #self.centroids=self.manifold.expmap0(self.centroids,c=1.0)
        #    return
        ...
        self.init_centroids(dim = X.shape[1])
        # print('yes')
        # print(self.centroids)
        if y is not None:
            self.init_assign(y)
...
def plot_clusters(emb, labels, centroids, edge_list, title=None, height=8, width=8,
    ...
    plt.ylim([-1.2,1.2])
    ax = plt.gca()
    circ = plt.Circle((0, 0), radius=1.0, edgecolor='black', facecolor='None', linewidth=3, alpha=0.5)
    ax.add_patch(circ)
    ...
    for i in range(n_clusters):
        plt.scatter(emb_data[(labels[:, i] == 1), 0], emb_data[(labels[:, i] == 1), 1],
                             color = colors[i], alpha=0.8, edgecolors='w', linewidth=2, s=100)
        plt.scatter(centroids[i, 0], centroids[i, 1], s=200, color = colors[i],
    ...
    for i in range(int(len(edge_list) * edge_frac)):
        ...
        _ = plt.plot([x1[0], x2[0]], [x1[1], x2[1]], '--', c='black', linewidth=1, alpha=0.1)
